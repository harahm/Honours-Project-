...
from hubcarapp.models import Garage, Item, Order, OrderDetails, Driver
from hubcarapp.serializers import GarageSerializer, \
    ItemSerializer, \
    OrderSerializer

import stripe
from hubcar.settings import STRIPE_API_KEY
...
def customer_get_garages(request):
    garages = GarageSerializer(
        Garage.objects.all().order_by("-id"),
        many = True,
        context = {"request": request}
    ).data

    return JsonResponse({"garages": garages})
    ...
    if request.method == "POST":
        ...
        if len(order_details) > 0:
            ...
            charge = stripe.Charge.create(
                amount = order_total * 100, # Amount in pounds
                currency = "gbp",
                source = stripe_token,
                description = "Hubcar Order"
            )
            ...
            if charge.status != "failed":
                ...
                order = Order.objects.create(
                    customer = customer,
                    garage_id = request.POST["garage_id"],
                    total = order_total,
...
def garage_order_notification(request, last_request_time):
    notification = Order.objects.filter(garage = request.user.garage,
